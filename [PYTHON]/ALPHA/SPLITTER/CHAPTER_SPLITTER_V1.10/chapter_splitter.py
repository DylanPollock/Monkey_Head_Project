...
class ChapterSplitter:
    ...
    def initial_message(self):
        ...
        print(f"Monkey Head Project: Gizmo / BananaBrain - {formatted_program_name}", "initialization")
    ...
    def get_user_selection(self, text_files):
        if not text_files:
            ...
            self.print_message("Invalid input to get_user_selection function. Gracefully exiting.", "error")
            sys.exit()
        while True:
            try:
    ...
    def parse_file_name(self, file_name):
        ...
        if len(parts) == 3:
            self.print_message(f"'{file_name.replace('.txt', '')}' is a valid file for information extraction. No additional information needed.", "info")
    ...
    def digit_check(self, txt_name):
        ...
        if failed_lines == 0 and is_sequential:
            ...
            self.print_message("Algorithm detects not sequential lines or text has failed line check.", "warning")
            self.print_message("The text file has failed some digit format checks.", "warning")
            return False
    ...
    def format_attempt(self, file_name):
        ...
        if final_digit_result:
            ...
            self.print_message("Function 'format_attempt' check has failed.", "warning")
            return False

    def check_format(self, new_file_name):
        self.print_message("Format check is currently underway...", "status")
        digit_result = self.digit_check(new_file_name)
        
        if digit_result:
            self.print_message("Format check suggest the file is already properly formatted.", "status")
            return True
        else:
            self.print_message("Algorithm detected malformed lines.", "warning")
            self.print_message("Function 'check_format' has failed.", "warning")
            return self.format_attempt(new_file_name)
    ...
    def correct_format(self, file_path):
        ...
        if malformed_lines >= prev_malformed_lines:
            ...
            self.print_message("Function 'correct_format' has failed.", "warning")
            return False
        else:
